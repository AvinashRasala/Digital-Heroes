...
function validateLead(body) {
  ...
  if (!BUDGET_RANGES.includes(budget_range)) errors.budget_range = 'Choose a valid budget range.';
  if (message.length > 2000) errors.message = 'Message is too long (max 2000 characters).';

  return { valid: Object.keys(errors).length === 0, errors, clean: { name, email, budget_range, message } };
}
...
app.post('/api/leads', (req, res) => {
  ...
  const info = stmt.run(clean.name, clean.email, clean.budget_range, clean.message);
...
app.get('/api/session', (req, res) => {
  res.json({ loggedIn: !!(req.session && req.session.adminId), username: req.session?.username || null });
});
...
app.get('/api/admin/leads', requireAuth, (req, res) => {
  ...
  if (q) {
    const like = `%${q}%`;
    rows = db.prepare(
      `SELECT * FROM leads WHERE name LIKE ? OR email LIKE ? OR message LIKE ? ORDER BY created_at DESC`
    ).all(like, like, like);
  } else {
...
// ---------- Admin page guard (server-side, not just client-side) ----------
...
app.listen(PORT, () => console.log(`LeadDesk Mini running on http://localhost:${PORT}`));
